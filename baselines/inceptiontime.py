...
OUT_DIR = r"baselines_out/inceptiontime_consistent_mb3"
SEED = 42
...
class RunLogger:
    def __init__(self, out_dir: str):
        self.lines: List[str] = []
        self.out_txt = os.path.join(out_dir, "itime_log.txt")
        self.out_json = os.path.join(out_dir, "itime_meta.json")
...
class InceptionBlock(nn.Module):
    def __init__(self, in_ch: int, nb_filters: int = 64, bottleneck: int = 32, kernel_sizes=(9,19,39)):
        super().__init__()
        ks = [_make_odd(k) for k in kernel_sizes]
        use_bneck = in_ch > 1
        self.bneck = nn.Conv1d(in_ch, bottleneck, kernel_size=1, bias=False) if use_bneck else nn.Identity()
        bch = bottleneck if use_bneck else in_ch
        self.conv1 = nn.Conv1d(bch, nb_filters, kernel_size=ks[0], padding=ks[0]//2, bias=False)
        self.conv2 = nn.Conv1d(bch, nb_filters, kernel_size=ks[1], padding=ks[1]//2, bias=False)
        self.conv3 = nn.Conv1d(bch, nb_filters, kernel_size=ks[2], padding=ks[2]//2, bias=False)
    ...
    def forward(self, x):
        ...
        z2 = self.conv2(z)
        z3 = self.conv3(z)
        z4 = self.conv_pool(self.maxpool(x))
        out = torch.cat([z1, z2, z3, z4], dim=1)
        out = F.relu(self.bn(out))
        return out

class InceptionTime(nn.Module):
    def __init__(self, in_ch: int, n_classes: int, n_groups: int = 3, nb_filters: int = 64):
        super().__init__()
        self.groups = nn.ModuleList()
        self.res_convs = nn.ModuleList()
        ch_in = in_ch
        for _ in range(n_groups):
            b1 = InceptionBlock(ch_in,          nb_filters=nb_filters)
            b2 = InceptionBlock(4*nb_filters,   nb_filters=nb_filters)
            b3 = InceptionBlock(4*nb_filters,   nb_filters=nb_filters)
            self.groups.append(nn.ModuleList([b1, b2, b3]))
            if ch_in != 4*nb_filters:
                self.res_convs.append(nn.Sequential(
                    nn.Conv1d(ch_in, 4*nb_filters, kernel_size=1, bias=False),
                    nn.BatchNorm1d(4*nb_filters)
                ))
            else:
                self.res_convs.append(nn.Identity())
            ch_in = 4*nb_filters
        self.bn_last = nn.BatchNorm1d(ch_in)
        self.gap = nn.AdaptiveAvgPool1d(1)
        self.head = nn.Linear(ch_in, n_classes)
    def forward(self, x):
        for grp, resmap in zip(self.groups, self.res_convs):
            res = resmap(x)
            x = grp[0](x)
            x = grp[1](x)
            x = grp[2](x)
...
def train_inception(model, train_loader, epochs=EPOCHS):
    model.to(DEVICE)
...
def main():
    ...
    train_loader = DataLoader(WindowDataset(X_train_win, y_train, None),
                              batch_size=BATCH_SIZE, shuffle=True, num_workers=0, drop_last=False)
    in_ch = X_train_win[0].shape[0] if X_train_win else len(canon_cols)
    n_classes = len(set(y_train))
    L.log(f"[Info] InceptionTime: in_ch={in_ch}, classes={n_classes}, train_windows={len(X_train_win)}")
    model = InceptionTime(in_ch=in_ch, n_classes=n_classes, n_groups=3, nb_filters=64)
    model = train_inception(model, train_loader, epochs=EPOCHS)
    model_path = os.path.join(OUT_DIR, "inceptiontime_model.pt")
    torch.save({"state_dict": model.state_dict(),
                "in_ch": in_ch, "n_classes": n_classes,
                "arch": {"n_groups": 3, "nb_filters": 64}}, model_path)
    ...
    for cond, (Xc, yc) in test_sets.items():
        ...
        L.log(f"  INCEPTIONTIME | {cond:20s} acc_w={acc_w:.3f} f1_w={f1_w:.3f} | acc_t={acc_t:.3f} f1_t={f1_t:.3f}")
    ensure_outdir(OUT_DIR)
    out_csv = os.path.join(OUT_DIR, "results_inceptiontime_CONSISTENT.csv")
    import csv
    with open(out_csv, "w", newline="", encoding="utf-8") as f:
        ...
        for cond, accw, f1w, acct, f1t in results:
            w.writerow(["inceptiontime_consistent_mb3", cond, f"{accw:.4f}", f"{f1w:.4f}",
        ...
        if results:
            ...
            w.writerow(["inceptiontime_consistent_mb3", "OVERALL_MEAN",
