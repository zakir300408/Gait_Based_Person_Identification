...
MB3_BRANCHES = ["raw","diff","diff2"]

DEFAULT_KERNELS = {"raw": 10000, "diff": 10000, "diff2": 8000}

WIN_S_FIXED = 2.0
CHANNEL_SET = ["acc", "gyro", "acc+gyro"]
NORM_SET    = ["none", "demean_window", "zscore_window", "zscore_trial"]
GATE_SET    = [None, 0.2, 0.4]

def _pre_name(ch, norm, gate, win_s=WIN_S_FIXED):
    gate_str = "noGate" if gate is None else f"G{gate:.1f}"
    norm_map = {"none":"none","demean_window":"demean","zscore_window":"zwin","zscore_trial":"ztrial"}
    return f"W{win_s:.1f}_{ch}_{norm_map[norm]}_{gate_str}"

def build_preproc_grid() -> List[Dict[str, Any]]:
    default = {"channels":"acc+gyro","norm":"zscore_window","gate_std_svm_min":0.4,"win_s":WIN_S_FIXED}
    default["name"] = _pre_name(default["channels"], default["norm"], default["gate_std_svm_min"])
    grid = [default]
    for ch in CHANNEL_SET:
        for norm in NORM_SET:
            for gate in GATE_SET:
                cfg = {"channels":ch, "norm":norm, "gate_std_svm_min":gate, "win_s":WIN_S_FIXED}
                cfg["name"] = _pre_name(ch, norm, gate)
                if cfg["name"] == default["name"]:
                    continue
                grid.append(cfg)
    return grid

PREPROC_EXPERIMENTS = build_preproc_grid()
...
def segment_windows_with_preproc(
    ...
    keep_idx = []
    if use_acc: keep_idx += acc_idx
    if use_gyro: keep_idx += gyr_idx
    if not keep_idx:
        return [], []
    mu_trial = None; sd_trial = None
...
def main():
    ...
    L.add_meta("branches_final", MB3_BRANCHES)
    ...
    try:
        import sktime; env["sktime"] = sktime.__version__
    except Exception:
        env["sktime"] = "unknown"
    L.add_meta("env", env)
    ...
    with h5py.File(H5_PATH, "r") as h5:
        ...
        preproc_rows_overall = []
        best_cond_rows = []
        best_overall = (-1.0, None)
        baseline_key = None
        perexp_trial_ok: Dict[str, Dict[str,int]] = {}
        for pi, pre in enumerate(PREPROC_EXPERIMENTS):
            pre_name = pre["name"]
            fs = 100
            win = int(float(pre["win_s"]) * fs)
            hop = win // 2
            ...
            for s in keep_subjects:
                for i in rows_by[(s, "baseline")]:
                    ...
                    X_train_win.extend(wins); train_meta.extend(metas)
            ...
            test_sets = {}; test_meta_by_cond: Dict[str, List[Dict[str,Any]]] = {}
            for cond in eval_conds:
                ...
                for i,(s,c) in enumerate(zip(idx["subject"], idx["condition"])):
                    if s in keep_subjects and c == cond:
                        ...
                        Xc.extend(wins); metas_c.extend(metas)
                if Xc:
                    test_sets[cond] = Xc
            ...
            branch_models = {}
            for bname in MB3_BRANCHES:
                pre_fn = BRANCH_LIBRARY[bname]
                ...
                t0 = perf_counter(); mr.fit(Xb_train, y_train_arr)
            ...
            with open(out_summary, "a", newline="", encoding="utf-8") as fsum, \
                ...
                ws = csv.writer(fsum); wt = csv.writer(ftri); wsub = csv.writer(fsub)
                kernels_desc = ";".join([f"{b}:{DEFAULT_KERNELS[b]}" for b in MB3_BRANCHES])
                X_train_feat = np.concatenate([train_feats_by_branch[b] for b in MB3_BRANCHES], axis=1)
                total_F = int(sum(train_feats_by_branch[b].shape[1] for b in MB3_BRANCHES))
                L.log(f"\n[MB-3] {pre_name} | branches={MB3_BRANCHES} | total_F={total_F}")
                clf = Pipeline([
                ...
                for cond in test_sets:
                    feats_c_list = [test_feats_by_cond_branch[cond][b] for b in MB3_BRANCHES]
                    ...
                    for yt, yp, tid in zip(y_true_t, y_pred_t, t_ids):
                        wt.writerow([pre_name, "MB-3", cond, tid, id2label[yt], id2label[yp], int(1 if yt==yp else 0)])
                    ...
                    for s_name, acc in per_subj_acc.items():
                        wsub.writerow([pre_name, "MB-3", cond, s_name, trials_per_subj.get(s_name, 0), f"{acc:.6f}", f"{1.0-acc:.6f}"])
                    ...
                    ws.writerow([pre_name, "MB-3", kernels_desc, cond,
                                 f"{acc_w:.4f}", f"{f1_w:.4f}", f"{acc_t:.4f}", f"{f1_t:.4f}",
                                 f"{float(pre['win_s']):.1f}", SEED, X_train_nct.shape[0], total_F, "",
                                 pre["channels"], pre["norm"], "" if pre["gate_std_svm_min"] is None else f"{pre['gate_std_svm_min']:.1f}"])
                    L.log(f"  {cond:20s} | acc_w={acc_w:.3f} f1_w={f1_w:.3f} | acc_t={acc_t:.3f} f1_t={f1_t:.3f}")
                if per_cond_stats:
                    means = np.nanmean(np.array(per_cond_stats), axis=0)
                    ws.writerow([pre_name, "MB-3", kernels_desc, "OVERALL_MEAN",
                    ...
                    preproc_rows_overall.append({
                        "pre": pre_name,
                        "channels": pre["channels"],
                        "norm": pre["norm"],
                        "gate": ("—" if pre["gate_std_svm_min"] is None else f"{pre['gate_std_svm_min']:.1f}"),
                    ...
                    if np.isfinite(means[2]) and float(means[2]) > best_overall[0]:
                        best_overall = (float(means[2]), pre_name)
                        best_cond_rows = []
                        for cond in test_sets:
                            feats_c_list = [test_feats_by_cond_branch[cond][b] for b in MB3_BRANCHES]
                            Xc_feat = np.concatenate(feats_c_list, axis=1)
                ...
                exp_key = f"{pre_name}|MB-3"
                perexp_trial_ok[exp_key] = pertrial_ok
                L.add_exp_meta({
                    "preproc": pre_name, "exp": "MB-3", "branches": MB3_BRANCHES,
                    "kernels": {b:DEFAULT_KERNELS[b] for b in MB3_BRANCHES},
                    "total_features": total_F, "win_s": float(pre["win_s"]),
                    "channels": pre["channels"], "norm": pre["norm"], "gate": pre["gate_std_svm_min"]
                })
                if baseline_key is None:
                    baseline_key = exp_key
    ...
    def _norm_rank(n):
        order = {"none":0,"demean_window":1,"zscore_window":2,"zscore_trial":3}
        return order.get(n, 99)
    def _ch_rank(c):
        order = {"acc":0,"gyro":1,"acc+gyro":2}
        return order.get(c, 99)
    def _gate_rank(g):
        if g == "—": return -1
        try: return int(float(g)*10)
        except: return 99
    preproc_rows_sorted = sorted(
        preproc_rows_overall,
        key=lambda r: (_ch_rank(r["channels"]), _norm_rank(r["norm"]), _gate_rank(r["gate"]))
    )
    best_cond_rows_sorted = sorted(best_cond_rows, key=lambda r: r["cond"])
    def row_pre(r):
        nm = r["norm"].replace("_","\\_")
        return (f"{r['channels']} & {nm} & {r['gate']} & "
                f"{r['acc_t']} & {r['f1_t']} & {r['acc_w']} & {r['f1_w']} & "
                f"{r['mis_trials']} & {r['mis_subjects']} & {r['features']} \\\\")
    def row_cond(r):
        return (f"{latex_escape(r['cond'])} & {r['acc_t']} & {r['f1_t']} & "
                f"{r['acc_w']} & {r['f1_w']} & {r['mis_trials']} & {r['mis_subjects']} \\\\")
    body_pre  = "\n".join([row_pre(r) for r in preproc_rows_sorted])
    body_cond = "\n".join([row_cond(r) for r in best_cond_rows_sorted])
    table_pre = r"""
% ===== Table 1: Preprocessing ablation for MB-3 (fixed window = 2.0 s) =====
\begin{table}[t]
  \caption{Preprocessing ablation for MB--3 (raw+diff+diff2) at fixed 2.0\,s windows. Reported are overall means across all non-baseline conditions. Gate uses accel SVM std; “—” = no gate. Values in percent.}
  \label{tab:preproc_ablation_mb3}
  \centering
  \footnotesize
  \renewcommand{\arraystretch}{1.12}
  \setlength{\tabcolsep}{4pt}
  \begin{tabular}{C{1.9cm} C{2.3cm} C{0.9cm} C{1.6cm} C{1.6cm} C{1.6cm} C{1.6cm} C{1.6cm} C{2.1cm} C{1.6cm}}
    \toprule
    \textbf{Channels} & \textbf{Normalization} & \textbf{Gate} & \textbf{Acc. (trial)} & \textbf{F1 (trial)} & \textbf{Acc. (window)} & \textbf{F1 (window)} & \textbf{MisID trials} & \textbf{Subjects w/ misID} & \textbf{Features} \\
    \midrule
""" + body_pre + r"""
    \bottomrule
  \end{tabular}
\end{table}
""".strip()
    best_label = "(best by Acc. trial)"
    table_best = r"""
% ===== Table 2: Condition-wise results for best preprocessing (2.0 s) =====
\begin{table}[t]
  \caption{MB--3 condition-wise performance for the best preprocessing """ + best_label + r""". Values in percent. MisID trials: trials where majority-vote prediction mismatched the true subject; ``Subjects w/ misID'': unique subjects with at least one misidentified trial in that condition.}
  \label{tab:best_preproc_condition}
  \centering
  \footnotesize
  \renewcommand{\arraystretch}{1.12}
  \setlength{\tabcolsep}{4pt}
